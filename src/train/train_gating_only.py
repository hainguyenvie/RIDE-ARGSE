...
def build_group_priors(expert_names, K, head_boost=1.5, tail_boost=1.5):
    ...
    HYBRID RIDE: 9 experts from 3 models with different loss functions.
    - CE experts (0-2): Better on head classes
    - LogitAdjust experts (3-5): Balanced
    - BalancedSoftmax experts (6-8): Better on tail classes
    ...
    # Keywords for loss-based specialization
    head_keywords = ['ce']  # CE models are head-biased
    tail_keywords = ['balsoftmax']  # BalSoftmax models are tail-focused
    
    for e, name in enumerate(expert_names):
        lname = name.lower()
        
        # Boost head-biased experts for head group
        if any(k in lname for k in head_keywords):
            pi[0, e] *= head_boost
        
        # Boost tail-focused experts for tail group
    ...
    print(f"   HYBRID RIDE: 9 experts from 3 models (CE, LA, BS)")
    for k in range(K):
        group_name = "head" if k == 0 else "tail"
        top_experts = torch.argsort(pi[k], descending=True)[:3]
        print(f"   {group_name} top 3: {[expert_names[i] for i in top_experts]}")
        print(f"   {group_name} priors: CE={pi[k, 0:3].mean():.3f}, LA={pi[k, 3:6].mean():.3f}, BS={pi[k, 6:9].mean():.3f}")
